...
public class ImageUploadServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			// 3. 요청파라미터 조회작업
			List<FileItem> list = upload.parseRequest(request); // 요청파라미터들을
																// list에 담아서
																// 전달해준다.
			...
			for (FileItem item : list) {
				...
				if (item.isFormField()) {// true: 일반 요청 파라미터(String)
					String reqValue = item.getString("utf-8");// 요청파라미터 조회. 인코딩
																// 설정 반드시 해야한다.
